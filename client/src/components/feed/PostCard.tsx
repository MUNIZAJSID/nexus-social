...
export const PostCard: React.FC<PostCardProps> = ({ post, onPostDeleted }) => {
  ...
  // Pré-carregamento imediato de todas as fotos do post em segundo plano
  useEffect(() => {
    if (mediaList.length > 1) {
      mediaList.forEach((m) => {
        if (m.mediaType === 'IMAGE') {
          const img = new Image();
          img.src = getMediaUrl(m.url);
        }
      });
    }
  }, [mediaList]);
  ...
  const handleTouchEnd = () => {
    ...
    if (distance > minSwipeDistance) {
      // Deslizar para esquerda -> Próxima foto
      if (currentMediaIndex < mediaList.length - 1) {
        setCurrentMediaIndex((prev) => prev + 1);
      }
    } else if (distance < -minSwipeDistance) {
      // Deslizar para direita -> Foto anterior
  ...
  return (
    ...
    >
      ...
      <header className="flex items-center justify-between px-4 py-3.5 sm:px-5">
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-1.5">
              <Link
                ...
                className="font-bold text-sm text-slate-900 dark:text-slate-100 hover:underline truncate"
              >
        ...
        {/* Menu de Ações (Excluir/Denunciar) */}
        <div className="relative">
          ...
          {showMenu && (
            <div className="absolute right-0 mt-1 w-36 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 py-1.5 z-30 animate-scale-fade">
              {canDelete && (
                <button
      ...
      {/* Container de Mídia Deslizante Instantâneo (Fotos e Vídeos) */}
      <div
      ...
      >
        {/* Carrossel Deslizante Hardware-Accelerated Instantâneo */}
        <div
          ...
          {mediaList.map((mediaItem, idx) => {
            ...
            return (
              <div
                ...
                className="relative w-full h-full flex-shrink-0 flex items-center justify-center bg-black overflow-hidden"
              >
                {isVideo ? (
                  <div
                  ...
                  >
                    <video
                      ref={idx === currentMediaIndex ? videoRef : undefined}
                      ...
                      playsInline
                      preload="metadata"
                      onTimeUpdate={idx === currentMediaIndex ? handleTimeUpdate : undefined}
                    ...
                    {showPlayOverlay && idx === currentMediaIndex && (
        ...
        {hasMultipleMedia && (
          <>
            {currentMediaIndex > 0 && (
              <button
                onClick={prevMedia}
                className="absolute left-3 top-1/2 -translate-y-1/2 p-2 bg-black/50 hover:bg-black/75 text-white rounded-full shadow-lg transition-transform hover:scale-105 opacity-80 group-hover:opacity-100 z-10 cursor-pointer"
              >
            ...
            {currentMediaIndex < mediaList.length - 1 && (
              <button
                onClick={nextMedia}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-2 bg-black/50 hover:bg-black/75 text-white rounded-full shadow-lg transition-transform hover:scale-105 opacity-80 group-hover:opacity-100 z-10 cursor-pointer"
              >
